...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Promocion_x_ItemProducto')
BEGIN
...
SELECT DISTINCT 
    ...
    [PROMOCION_FECHA_FIN],
    (SELECT TOP 1 id_regla 
     FROM Regla 
     WHERE id_promocion_prod = [PROMO_CODIGO])
FROM gd_esquema.Maestra
...
SELECT DISTINCT 
    (SELECT TOP 1 id_detalle_pago FROM Detalle_Pago WHERE nro_tarjeta = [PAGO_TARJETA_NRO] AND id_cliente = (SELECT id_cliente FROM Cliente WHERE dni_cliente = [CLIENTE_DNI])), 
    [PAGO_FECHA], 
    [PAGO_IMPORTE], 
...
SELECT DISTINCT 
    [TICKET_NUMERO], 
    (SELECT TOP 1 codigo_producto FROM Producto WHERE prod_nombre = [PRODUCTO_NOMBRE] AND id_prod_marca = (SELECT id_prod_marca FROM Marca_Producto WHERE nombre_prod_marca = [PRODUCTO_NOMBRE])), 
    [TICKET_DET_CANTIDAD], 
    [TICKET_DET_PRECIO], 
    [TICKET_DET_TOTAL]
FROM gd_esquema.Maestra
...
SELECT DISTINCT 
    [PROMO_CODIGO], 
    (SELECT TOP 1 codigo_producto FROM Producto WHERE prod_nombre = [PRODUCTO_NOMBRE] AND id_prod_marca = (SELECT TOP 1 id_prod_marca FROM Marca_Producto WHERE nombre_prod_marca = [PRODUCTO_MARCA])), 
    [TICKET_NUMERO]
FROM gd_esquema.Maestra
WHERE PROMO_CODIGO IS NOT NULL;
